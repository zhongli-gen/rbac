...
public class RedisUtil {
    ...
    private final String DEFAULT_KEY_PREFIX = "";   // key的前缀
    private final int EXPIRE_TIME = 1;              // key的过期时间
    private final TimeUnit EXPIRE_TIME_TYPE = TimeUnit.DAYS;    // key过期的时间单位
    ...
    public <K, V> void add(K key, V value) {
        try {
            if (value != null) {
                redisTemplate
                        .opsForValue()
                        .set(DEFAULT_KEY_PREFIX + key, new ObjectMapper().writeValueAsString(value));
            }
        } catch (Exception e) {
    ...
    public <K, V> void add(K key, V value, long timeout, TimeUnit unit) {
        try {
            if (value != null) {
                redisTemplate
                        .opsForValue()
                        .set(DEFAULT_KEY_PREFIX + key, new ObjectMapper().writeValueAsString(value), timeout, unit);
            }
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            throw new RuntimeException("数据缓存至redis失败");
    ...
    public <K, V> V getObject(K key, Class<V> clazz) {
        ...
        if (!StringUtils.isEmpty(value)) {
            try {
                result = new ObjectMapper().readValue(value, clazz);
    ...
    public <K, V> List<V> getList(K key, Class<V> clazz) {
        ...
        if (!StringUtils.isEmpty(value)) {
            try {
                result = new ObjectMapper().readValue(value, new TypeReference<List<V>>() {});
            } catch (JsonProcessingException e) {
                e.printStackTrace();
            }
        } else {
